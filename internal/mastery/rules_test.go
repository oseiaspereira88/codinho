...
func TestAdvanceStateFirstContactAlwaysIntroduces(t *testing.T) {
	cases := []Signal{
		{ContentProvenance: "published", Success: false, Observed: day(1)},
		{ContentProvenance: "published", Success: true, SolutionRevealed: true, Observed: day(1)},
	}
...
func TestAdvanceStateGuidedSuccessNeverExceedsDemonstratesWithHelp(t *testing.T) {
	...
	for i := range 10 {
		proj = AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, HelpUsed: true, Observed: day(1 + i)})
	}
...
func TestAdvanceStateAutonomousSuccessReachesDemonstratesWithoutHelpDirectly(t *testing.T) {
	proj := AdvanceState(RuleVersionV1, DimensionProjection{}, Signal{ContentProvenance: "published", Success: true, HelpUsed: false, Observed: day(1)})
...
func TestAdvanceStateSolutionRevealedNeverPromotes(t *testing.T) {
	...
	got := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, HelpUsed: false, SolutionRevealed: true, Observed: day(1)})
...
func TestAdvanceStateFailureNeverRegresses(t *testing.T) {
	...
	got := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: false, Observed: day(1)})
...
func TestAdvanceStateRetentionRequiresADifferentCalendarDay(t *testing.T) {
	proj := AdvanceState(RuleVersionV1, DimensionProjection{}, Signal{ContentProvenance: "published", Success: true, Observed: day(1)})
	if proj.State != StateDemonstratesWithoutHelp {
		t.Fatalf("precondition failed: state = %s", proj.State)
	}

	sameDay := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, Observed: day(1).Add(2 * time.Hour)})
	...
	laterDay := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, Observed: day(2)})
...
func TestAdvanceStateTransferRequiresAnUnseenVariant(t *testing.T) {
	proj := AdvanceState(RuleVersionV1, DimensionProjection{}, Signal{ContentProvenance: "published", Success: true, Variant: "v1", Observed: day(1)})
	proj = AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, Variant: "v1", Observed: day(2)})
	if proj.State != StateRetained {
		t.Fatalf("precondition failed: state = %s", proj.State)
	}

	sameVariant := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, Variant: "v1", Observed: day(3)})
	if sameVariant.State != StateRetained {
		t.Fatalf("same variant: state = %s, want still retained", sameVariant.State)
	}

	newVariant := AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: true, Variant: "v2", Observed: day(3)})
...
func TestAdvanceStateEvidenceCountAlwaysIncrements(t *testing.T) {
	proj := DimensionProjection{}
	for i := 1; i <= 5; i++ {
		proj = AdvanceState(RuleVersionV1, proj, Signal{ContentProvenance: "published", Success: false, Observed: day(i)})
