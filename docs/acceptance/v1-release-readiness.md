...
|---|---|---|
| R1 | Instalação verificada e gates POSE estritos | Ferramentas verificadas; gates de estrutura/skills passam, validate falha no catálogo. |
| R2 | Matriz única, race, formatação, contratos e fixtures | 18 de 19 checks passam; fixtures de rascunhos bloqueiam o catálogo. |
...
| R5 | Proveniência reconciliada, roots e contratos | Pendente. |
...
`b6b90ec9ce35` com alterações no worktree. Resultado: fail no catálogo;
18 checks pass. Isso não é uma atestação de um commit imutável.
...
continuam bloqueados por artefatos/proveniência pendentes.
